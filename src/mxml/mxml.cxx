/// \file mxml.cxx
/// \brief Midas XML Library
/// \details This is a simple implementation of XML functions for writing and
...
/********************************************************************\
   ...
   $Id: mxml.c 72 2012-02-03 15:50:14Z ritt $
...
/*
	...
	are as follows:
	 - Extension changed to .cxx (intended for C++ compilation).
	 - All functions placed under the `rb` namespace.
	 - Doxygen documentation header added
*/
...
void *rb::mxml_malloc(size_t size)
{
   return malloc(size);
}
...
void *rb::mxml_realloc(void *p, size_t size)
{
...
int rb::mxml_write_line(MXML_WRITER *writer, const char *line)
{
   int len;
 ...
 */
rb::MXML_WRITER *rb::mxml_open_buffer(void)
{
 ...
 */
void rb::mxml_suppress_date(int suppress)
{
 ...
 */
rb::MXML_WRITER *rb::mxml_open_file(const char *file_name) 
{
   char str[256], line[1000];
   time_t now;
   MXML_WRITER *writer;
 ...
 * convert '<' '>' '&' '"' ''' into &xx;
 */
void rb::mxml_encode(char *src, int size, int translate)
{
   char *ps, *pd;
   ...
   *pd = 0;
   ...
   rb::strlcpy(src, _encode_buffer, size);
}
 ...
 */
int rb::mxml_set_translate(MXML_WRITER *writer, int flag)
{
   int old_flag;
 ...
 */
int rb::mxml_start_element1(MXML_WRITER *writer, const char *name, int indent)
{
   int i;
   ...
   if (indent)
      for (i=0 ; i<writer->level ; i++)
         rb::strlcat(line, XML_INDENT, sizeof(line));
   rb::strlcat(line, "<", sizeof(line));
   rb::strlcpy(name_enc, name, sizeof(name_enc));
   mxml_encode(name_enc, sizeof(name_enc), writer->translate);
   rb::strlcat(line, name_enc, sizeof(line));
...
int rb::mxml_start_element(MXML_WRITER *writer, const char *name)
{
...
int rb::mxml_start_element_noindent(MXML_WRITER *writer, const char *name)
{
 ...
 */
int rb::mxml_end_element(MXML_WRITER *writer)
{
   int i;
   char line[1000];
   ...
   rb::strlcat(line, "</", sizeof(line));
   rb::strlcat(line, writer->stack[writer->level], sizeof(line));
   mxml_free(writer->stack[writer->level]);
   if (writer->level == 0)
      mxml_free(writer->stack);
   rb::strlcat(line, ">\n", sizeof(line));
 ...
 */
int rb::mxml_write_attribute(MXML_WRITER *writer, const char *name, const char *value)
{
 ...
 */
int rb::mxml_write_value(MXML_WRITER *writer, const char *data)
{
 ...
 * write empty line
 */
int rb::mxml_write_empty_line(MXML_WRITER *writer)
{
   if (writer->element_is_open) {
      mxml_write_line(writer, ">\n");
      writer->element_is_open = FALSE;
 ...
 */
int rb::mxml_write_comment(MXML_WRITER *writer, const char *string)
{
   int  i;
   char line[1000];
   ...
   rb::strlcat(line, "<!-- ", sizeof(line));
   rb::strlcat(line, string, sizeof(line));
   rb::strlcat(line, " -->\n", sizeof(line));
 ...
 */
int rb::mxml_write_element(MXML_WRITER *writer, const char *name, const char *value)
{
   int i;
 ...
 */
char *rb::mxml_close_buffer(MXML_WRITER *writer)
{
   int i;
   char *p;
 ...
 * close a file opened with mxml_open_writer
 */
int rb::mxml_close_file(MXML_WRITER *writer)
{
   int i;
 ...
 */
rb::PMXML_NODE rb::mxml_create_root_node(void)
{
   rb::PMXML_NODE root;

   root = (rb::PMXML_NODE)calloc(sizeof(MXML_NODE), 1);
 ...
 */
rb::PMXML_NODE rb::mxml_add_special_node_at(rb::PMXML_NODE parent, int node_type, const char *node_name, const char *value, int idx)
{
   rb::PMXML_NODE pnode, pchild;
   int i, j;

   assert(parent);
   if (parent->n_children == 0)
      parent->child = (rb::PMXML_NODE)mxml_malloc(sizeof(MXML_NODE));
   else
      parent->child = (rb::PMXML_NODE)mxml_realloc(parent->child, sizeof(MXML_NODE)*(parent->n_children+1));
   assert(parent->child);
   ...
   rb::strlcpy(pnode->name, node_name, sizeof(pnode->name));
 ...
 */
rb::PMXML_NODE rb::mxml_add_special_node(rb::PMXML_NODE parent, int node_type, const char *node_name, const char *value)
{
 ...
 * write value of an XML element, like <[name]>[value]</[name]>
 */
rb::PMXML_NODE rb::mxml_add_node(rb::PMXML_NODE parent, const char *node_name, const char *value)
{
 ...
 * add a subnode (child) to an existing parent node at the end
 */
rb::PMXML_NODE rb::mxml_add_node_at(rb::PMXML_NODE parent, const char *node_name, const char *value, int idx)
{
 ...
 */
int rb::mxml_add_tree_at(rb::PMXML_NODE parent, rb::PMXML_NODE tree, int idx)
{
   rb::PMXML_NODE pchild;
   int i, j, k;
 ...
 */
int rb::mxml_add_tree(rb::PMXML_NODE parent, rb::PMXML_NODE tree)
{
 ...
 */
int rb::mxml_add_attribute(rb::PMXML_NODE pnode, const char *attrib_name, const char *attrib_value)
{
   ...
   rb::strlcpy(pnode->attribute_name+pnode->n_attributes*MXML_NAME_LENGTH, attrib_name, MXML_NAME_LENGTH);
 ...
 */
int rb::mxml_get_number_of_children(rb::PMXML_NODE pnode)
{
   assert(pnode);
 ...
 * return number of subnodes (children) of a node
 */
rb::PMXML_NODE rb::mxml_subnode(rb::PMXML_NODE pnode, int idx)
{
   assert(pnode);
...
namespace rb { int mxml_find_nodes1(rb::PMXML_NODE tree, const char *xml_path, rb::PMXML_NODE **nodelist, int *found); }

int rb::mxml_add_resultnode(rb::PMXML_NODE node, const char *xml_path, rb::PMXML_NODE **nodelist, int *found)
{
   /* if at end of path, add this node */
   if (*xml_path == 0) {
      if (*found == 0)
         *nodelist = (rb::PMXML_NODE *)mxml_malloc(sizeof(rb::PMXML_NODE));
      else
         *nodelist = (rb::PMXML_NODE *)mxml_realloc(*nodelist, sizeof(rb::PMXML_NODE)*(*found + 1));
...
*/
int rb::mxml_find_nodes1(rb::PMXML_NODE tree, const char *xml_path, rb::PMXML_NODE **nodelist, int *found)
{
   rb::PMXML_NODE pnode;
   const char *p1,*p2;
      ...
      while (*p2 == '[') {
         ...
         if (isdigit(*p2)) {
            ...
            while (*p2 && isspace((unsigned char)*p2))
               p2++;
            rb::strlcpy(condition, p2, sizeof(condition));
            if (strchr(condition, ']'))
               *strchr(condition, ']') = 0;
            else
               return 0;
            ...
            if ((p3 = strchr(condition, '=')) != NULL) {
               if (condition[0] == '@') {
                  ...
                  rb::strlcpy(cond_name[num_cond], &condition[1], sizeof(cond_name[num_cond]));
               } else {
                  rb::strlcpy(cond_name[num_cond], condition, sizeof(cond_name[num_cond]));
               }
               ...
               *strchr(cond_name[num_cond], '=') = 0;
               ...
               while (*p3 && isspace(*p3))
                  p3++;
               if (*p3 == '\"') {
                  rb::strlcpy(cond_value[num_cond], p3+1, sizeof(cond_value[num_cond]));
                  ...
                  rb::strlcpy(cond_value[num_cond], p3, sizeof(cond_value[num_cond]));
                  while (cond_value[num_cond][0] && isspace(cond_value[num_cond][strlen(cond_value[num_cond])-1]))
                     cond_value[num_cond][strlen(cond_value[num_cond])-1] = 0;
               }
               num_cond++;
...
int rb::mxml_find_nodes(rb::PMXML_NODE tree, const char *xml_path, rb::PMXML_NODE **nodelist)
{
   int status, found = 0;
 ...
 */
rb::PMXML_NODE rb::mxml_find_node(rb::PMXML_NODE tree, const char *xml_path)
{
   rb::PMXML_NODE *node, pnode;
   int n;
...
char *rb::mxml_get_name(rb::PMXML_NODE pnode)
{
   assert(pnode);
   return pnode->name;
}
...
char *rb::mxml_get_value(rb::PMXML_NODE pnode)
{
   assert(pnode);
   return pnode->value;
}
...
int rb::mxml_get_line_number_start(rb::PMXML_NODE pnode)
{
   assert(pnode);
...
int rb::mxml_get_line_number_end(rb::PMXML_NODE pnode)
{
   assert(pnode);
...
char *rb::mxml_get_attribute(rb::PMXML_NODE pnode, const char *name)
{
   int i;
...
int rb::mxml_replace_node_name(rb::PMXML_NODE pnode, const char *name)
{
   rb::strlcpy(pnode->name, name, sizeof(pnode->name));
   return TRUE;
}

/*------------------------------------------------------------------*/

int rb::mxml_replace_node_value(rb::PMXML_NODE pnode, const char *value)
{
   if (pnode->value)
...
*/
int rb::mxml_replace_subvalue(rb::PMXML_NODE pnode, const char *name, const char *value)
{
   int i;
   ...
   return rb::mxml_replace_node_value(&pnode->child[i], value);
}
 ...
 */
int rb::mxml_replace_attribute_name(rb::PMXML_NODE pnode, const char *old_name, const char *new_name)
{
   int i;
   ...
   rb::strlcpy(pnode->attribute_name+i*MXML_NAME_LENGTH, new_name, MXML_NAME_LENGTH);
   return TRUE;
}
 ...
 */
int rb::mxml_replace_attribute_value(rb::PMXML_NODE pnode, const char *attrib_name, const char *attrib_value)
{
   int i;
 ...
 */
int rb::mxml_delete_node(rb::PMXML_NODE pnode)
{
   rb::PMXML_NODE parent;
   int i, j;
   ...
   if (parent) {
      ...
      if (i < parent->n_children) {
         ...
         if (parent->n_children)
            parent->child = (rb::PMXML_NODE)mxml_realloc(parent->child, sizeof(MXML_NODE)*(parent->n_children));
         else
            rb::mxml_free(parent->child);
      }
   } else 
      rb::mxml_free_tree(pnode);
...
int rb::mxml_delete_attribute(rb::PMXML_NODE pnode, const char *attrib_name)
{
   int i, j;
   ...
   rb::mxml_free(pnode->attribute_value[i]);
   ...
   if (pnode->n_attributes > 0) {
      pnode->attribute_name  = (char *)mxml_realloc(pnode->attribute_name,  rb::MXML_NAME_LENGTH*(pnode->n_attributes-1));
      pnode->attribute_value = (char **)mxml_realloc(pnode->attribute_value, sizeof(char *)*(pnode->n_attributes-1));
   } else {
      rb::mxml_free(pnode->attribute_name);
      rb::mxml_free(pnode->attribute_value);
   }
...
/**
 * used inside rb::mxml_parse_file for reporting errors
 */
namespace rb {
PMXML_NODE read_error(rb::PMXML_NODE root, const char *file_name, int line_number, char *error, int error_size, int *error_line, const char *format, ...)
{
   char *msg, str[1000];
   va_list argptr;
   ...
   if (error)
      rb::strlcpy(error, str, error_size);
   ...
   if (error)
      rb::strlcat(error, str, error_size);
   if (error_line)
      *error_line = line_number;
   
   rb::mxml_free(msg);
   rb::mxml_free_tree(root);
...
/**
 * Parse a XML buffer and convert it into a tree of rb::MXML_NODE's.
 * Return NULL in case of an error, return error description.
 * Optional file_name is used for error reporting if called from rb::mxml_parse_file()
 */
rb::PMXML_NODE rb::mxml_parse_buffer(const char *buf, char *error, int error_size, int *error_line)
{
   char node_name[256], attrib_name[256], attrib_value[1000], quote;
   const char *p, *pv;
   int i,j, line_number;
   rb::PMXML_NODE root, ptree, pnew;
   int end_element;
   size_t len;
   ...
   do {
      if (*p == '<') {
         ...
         if (strncmp(p, "!--", 3) == 0) {
            ...
            pnew = rb::mxml_add_special_node(ptree, COMMENT_NODE, "Comment", NULL);
            ...
            /* found ?...? element */
            pnew = rb::mxml_add_special_node(ptree, PROCESSING_INSTRUCTION_NODE, "PI", NULL);
            pnew->line_number_start = line_number;
            pv = p+1;
            ...
            if (end_element) {
               ...
               pnew = rb::mxml_add_node(ptree, node_name, NULL);
               pnew->line_number_start = line_number;
               pnew->line_number_end = line_number;
               ...
               while (*p != '>' && *p != '/') {
                  ...
                  attrib_value[len] = 0;
                  rb::mxml_decode(attrib_value);

                  /* add attribute to current node */
                  rb::mxml_add_attribute(pnew, attrib_name, attrib_value);
 ...
 * Optional file_name is used for error reporting if called from rb::mxml_parse_file()
 */
int rb::mxml_parse_entity(char **buf, const char *file_name, char *error, int error_size, int *error_line)
{
   char *p;
   char *pv;
   char delimiter;
   ...
   rb::PMXML_NODE root = rb::mxml_create_root_node();   /* dummy for 'HERE' */

   for (ip = 0; ip < rb::MXML_MAX_ENTITY; ip++)
   ...
   rb::mxml_dirname(directoryname);

   /* copy string to temporary space */
   buffer = (char *) rb::mxml_malloc(strlen(*buf) + 1);
   if (buffer == NULL) {
   ...
   do {
      ...
      if (*p == '<') {
         ...
         if (strncmp(p, "!--", 3) == 0) {
            ...
            replacement = (char *) rb::mxml_malloc(len + 1);
            if (replacement == NULL) {
               read_error(HERE, "Cannot allocate memory.");
               status = 1;
               goto error;
   ...
   for (i = 0; i < nentity; i++) {
      if (entity_type[i] == EXTERNAL_ENTITY) {
         if ( entity_reference_name[i][0] == MXML_DIR_SEPARATOR ) /* absolute path */
            strcpy(filename, entity_reference_name[i]);
         else /* relative path */
					 sprintf(filename, "%s%c%s", directoryname, MXML_DIR_SEPARATOR, entity_reference_name[i]);
         ...
         if (fh == -1) {
            ...
            lseek(fh, 0, SEEK_SET);
            if (length == 0) {
               entity_value[i] = (char *) rb::mxml_malloc(1);
               ...
               entity_value[i] = (char *) rb::mxml_malloc(length);
               if (entity_value[i] == NULL) {
                  read_error(HERE, "Cannot allocate memory.");
                  close(fh);
                  status = 1;
   ...
   /* re-allocate memory */
   *buf = (char *) rb::mxml_realloc(*buf, length + 1);
   if (*buf == NULL) {
      read_error(HERE, "Cannot allocate memory.");
      status = 1;
      goto error;
...
/**
 * parse a XML file and convert it into a tree of rb::MXML_NODE's.
 * Return NULL in case of an error, return error description
 */
rb::PMXML_NODE rb::mxml_parse_file(const char *file_name, char *error, int error_size, int *error_line)
{
   char *buf, line[1000];
   int fh, length;
   rb::PMXML_NODE root;
   ...
   if (fh == -1) {
      sprintf(line, "Unable to open file \"%s\": ", file_name);
      rb::strlcat(line, strerror(errno), sizeof(line));
      rb::strlcpy(error, line, error_size);
      return NULL;
   }
   ...
   root = rb::mxml_parse_buffer(buf, error, error_size, error_line);
...
/**
 * write complete subtree recursively into file opened with rb::mxml_open_document()
 */
int rb::mxml_write_subtree(MXML_WRITER *writer, rb::PMXML_NODE tree, int indent)
{
   int i;

   rb::mxml_start_element1(writer, tree->name, indent);
   ...
   return rb::mxml_end_element(writer);
}
 ...
 */
int rb::mxml_write_tree(const char *file_name, rb::PMXML_NODE tree)
{
   rb::MXML_WRITER *writer;
   int i;

   assert(tree);
   writer = rb::mxml_open_file(file_name);
   if (!writer)
      return FALSE;
...
rb::PMXML_NODE rb::mxml_clone_tree(rb::PMXML_NODE tree)
{
   rb::PMXML_NODE clone;
   int i;

   clone = (rb::PMXML_NODE)calloc(sizeof(MXML_NODE), 1);

   /* copy name, node_type, n_attributes and n_children */
   memcpy(clone, tree, sizeof(MXML_NODE));

   clone->value = NULL;
   rb::mxml_replace_node_value(clone, tree->value);

   clone->attribute_name = NULL;
   clone->attribute_value = NULL;
   for (i=0 ; i<tree->n_attributes ; i++)
      rb::mxml_add_attribute(clone, tree->attribute_name+i*rb::MXML_NAME_LENGTH, tree->attribute_value[i]);

   clone->child = NULL;
   clone->n_children = 0;
   for (i=0 ; i<tree->n_children ; i++)
      rb::mxml_add_tree(clone, rb::mxml_clone_tree(mxml_subnode(tree, i)));
 ...
 */
void rb::mxml_debug_tree(rb::PMXML_NODE tree, int level)
{
   int i, j;
   ...
   for (j=0 ; j<tree->n_attributes ; j++) {
      for (i=0 ; i<level ; i++)
         printf("  ");
      printf("%s: %s\n", tree->attribute_name+j*rb::MXML_NAME_LENGTH, 
   ...
   for (i=0 ; i<tree->n_children ; i++)
      rb::mxml_debug_tree(tree->child+i, level+1);
 ...
 * rb::mxml_create_root_node() or rb::mxml_parse_file()
 */
void rb::mxml_free_tree(rb::PMXML_NODE tree)
{
   int i;

   /* first free children recursively */
   for (i=0 ; i<tree->n_children ; i++)
      rb::mxml_free_tree(&tree->child[i]);
   if (tree->n_children)
      rb::mxml_free(tree->child);

   /* now free dynamic data */
   for (i=0 ; i<tree->n_attributes ; i++)
      rb::mxml_free(tree->attribute_value[i]);

   if (tree->n_attributes) {
      rb::mxml_free(tree->attribute_name);
      rb::mxml_free(tree->attribute_value);
   }
   
   if (tree->value)
      rb::mxml_free(tree->value);
...
{
   char err[256];
   rb::PMXML_NODE tree, tree2, node;

   tree = rb::mxml_parse_file("c:\\tmp\\test.xml", err, sizeof(err));
   tree2 = rb::mxml_clone_tree(tree);

   printf("Orig:\n");
   rb::mxml_debug_tree(tree, 0);

   printf("\nClone:\n");
   rb::mxml_debug_tree(tree2, 0);

   printf("\nCombined:\n");
   node = rb::mxml_find_node(tree2, "cddb"); 
   rb::mxml_add_tree(tree, node);
   rb::mxml_debug_tree(tree, 0);

   rb::mxml_free_tree(tree);
}
*/

/*------------------------------------------------------------------*/
 /**
   rb::mxml_basename deletes any prefix ending with the last slash '/' character
   present in path. rb::mxml_dirname deletes the filename portion, beginning with
...
void rb::mxml_basename(char *path)
{
...
void rb::mxml_dirname(char *path)
{
   char *p;
#ifdef _MSC_VER
   char *pv;
 ...
 */
rb::PMXML_NODE rb::mxml_get_node_at_line(rb::PMXML_NODE tree, int line_number)
{
   int i;
   rb::PMXML_NODE pn;
   ...
   for (i=0 ; i<tree->n_children ; i++) {
      pn = rb::mxml_get_node_at_line(&tree->child[i], line_number);
      if (pn)
         return pn;
   }
