...
void rb::AttachOnline(const char* host, const char* other, char** others, int nothers) {
  //  gBuffers->RunOnline(host, other, others, nothers);
...
void rb::AttachFile(const char* filename, Bool_t stop_at_end) {
  //  gBuffers->RunFile(filename, stop_at_end);
  ThreadFile * f = new ThreadFile(gSystem->ExpandPathName(filename), stop_at_end);
  f->Run(); 
}
...
rb::ThreadFile::ThreadFile(const char* filename, Bool_t stopAtEnd) :
  ThreadExecutor("AttachFile"), kFileName(filename), kStopAtEnd(stopAtEnd) {
